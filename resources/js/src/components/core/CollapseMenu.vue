<template>
  <b-collapse tag="ul" :class="className" :visible="open" :id="idName" :accordion="accordianName">
    <li v-show="abilities.includes('*') || abilities.includes(item.abilitie)" v-for="(item,index) in items" :key="index"
        :class=" !hideListMenuTitle? 'p-0' : item.is_heading ? 'iq-menu-title' :activeLink(item) && item.children ? 'active' : activeLink(item) ? 'active' : ''">
...
<<<<<<< HEAD:resources/js/src/components/core/menus/CollapseMenu.vue
import {core} from '../../../config/pluginInit'
import {mapState} from "vuex";

=======
import { core } from '../../config/pluginInit'
>>>>>>> c691d8263a4006061c6c8676464ede87d5956fd6:resources/js/src/components/core/CollapseMenu.vue
export default {
  name: 'List',
  props: {
    items: Array,
